...
function getClickedCardsInTabs(data=[]) {
    isBtnShowMoreDisplayed(data)
    var workingData = checkIsDataAvailable(data);
    var likeBtn, dislikeBtn;
    if($(window).width() >= 768) {
        likeBtn = $(workingData).map(function(inx, item) {
            return $(item).find(".featured-like-wrapper .featured-like");
       })
        dislikeBtn = $(workingData).map(function(inx, item) {
           return $(item).find(".featured-like-wrapper .featured-dislike");
      })
    }else{
        likeBtn = $(workingData).map(function(inx, item) {
            return $(item).find(".respo-voiting-section .featured-like");
       })
        dislikeBtn = $(workingData).map(function(inx, item) {
           return $(item).find(".respo-voiting-section .featured-dislike");
      })
    }
